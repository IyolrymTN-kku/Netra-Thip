// Icon library — minimal, line-based, 1.5px stroke, 16px default. SVG only.
// Usage: <Icon name="shield" size={16} />

const ICON_PATHS = {
  // Brand mark — ดวงตาทิพย์ "divine eye" stylized
...
function Icon({ name, size = 16, stroke = 1.5, className = '', style = {}, ...rest }) {
  const d = ICON_PATHS[name];
  if (!d) return null;
  // Some paths contain multiple subpaths via " M" — render as single d
  return (
    <svg
      width={size}
      height={size}
...
// Netra-Thip logo mark — Thai cyber identity
// Concept: stylized "ดวงตาทิพย์" (divine/third eye) — almond shape with concentric
// rings + crosshair. Combines Thai mandala geometry with technical scope reticle.
function NetraLogo({ size = 28, mono = false }) {
  const accent = mono ? 'currentColor' : 'var(--nt-blue)';
  const gold = mono ? 'currentColor' : 'var(--nt-gold)';
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" aria-label="Netra-Thip">
      {/* Outer petal/eye outline */}
      <path
      ...
      {/* Crosshair ticks */}
      <path d="M16 4 L16 7 M16 25 L16 28 M3 16 L6 16 M26 16 L29 16"
            stroke={accent} strokeWidth="1.4" strokeLinecap="round" />
      {/* Thai-inspired flame above (mini chofa) */}
      <path
