...
interface ToolReportData {
  project: { name: string; description: string | null; assessorName: string };
  scanJobInfo: {
    id: string;
    toolName: string;
    status: string;
...
export const PdfToolReport = forwardRef<HTMLDivElement, PdfToolReportProps>(
  ({ data }, ref) => {
    const { project, scanJobInfo, findings, kpi, compliance, topTargets } = data;
    const reportDate = new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" });
    const riskLabel = getRiskLabel(kpi.riskScore);
    const riskColor = getRiskColor(kpi.riskScore);
    const targets = [...new Set(findings.map((f: any) => f.target))];
      ...
      <div ref={ref} style={S.page}>
              ...
              <div style={{ padding: "18px" }}>
                <table style={{ ...S.table, marginBottom: "14px", fontSize: "12px" }}>
                  <tbody>
                    ...
                    {f.cves && Array.isArray(f.cves) && f.cves.length > 0 && (
                      <tr><td style={{ fontWeight: 600, color: "#64748B", padding: "4px 0" }}>CVEs:</td><td style={{ padding: "4px 0", fontFamily: "monospace", fontSize: "11px" }}>{f.cves.map((c: any) => c.cveId || c).join(", ")}</td></tr>
                    )}
                  </tbody>
                </table>
        ...
        <div style={S.section}>
          ...
          {["CRITICAL", "HIGH", "MEDIUM", "LOW"].map((sev) => {
            const sevFindings = findings.filter((f: any) => f.severity === sev && f.status !== "RESOLVED");
