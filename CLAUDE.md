...
- **BYOK (Bring Your Own Key) Encryption:** AI API Keys (OpenAI, Gemini, Claude) provided by users MUST be encrypted using `AES-256-GCM` before storing in PostgreSQL. The `iv` and `authTag` must be stored alongside the encrypted key. 
